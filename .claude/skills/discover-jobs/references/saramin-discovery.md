...
- ⚠️ **사람인은 연속 스크랩에 매우 민감하다.** 한 번에 빠르게 수십 건 이상 POST하면 **페이지 세션은 유지돼도 스크랩 ajax가 `error_code:1, "not-login"` 으로 일시 차단**된다(2026-06-23 실측: 134건 연속 → 전건 차단). **반드시 순차 + 건당 0.3~1초 지연**으로 호출하고, 1회 배치를 **30건 이하**로 제한하라. `not-login`이 나오면 즉시 멈추고 사용자에게 보고(쿨다운 필요 — 잠시 후 또는 재로그인 후 재시도).
