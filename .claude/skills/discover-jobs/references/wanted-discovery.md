...
3. 새 탭을 생성해 원티드 도메인을 한 번 연다(예: `https://www.wanted.co.kr`) — 사용자 기존 탭과의 간섭을 피하기 위함. 이후 모든 API는 이 탭에서 `fetch(..., {credentials:'include'})` 로 호출한다.
...
- 페이지네이션: `offset`을 limit(12)씩 늘리며 `total_count`까지. 검색어·사이트당 상한을 두고(예: offset 0~48, 최대 ~50건) 초과분은 잘라낸 건수를 리포트에 고지(SKILL.md 9단계 "리포트"의 조용한 누락 금지 원칙).
...
- 직무: `position`·`category_tag`·`detail` 텍스트에서 백엔드·AI·Agent·풀스택 키워드 매칭. **제외**: 프론트/모바일 전용, 데이터엔지니어, QA, 인프라·클라우드만 다루는 공고.
...
```
- 호출 후 검증: `jobs/v4/{jobId}/details` 의 `is_bookmark` 가 `true` 인지 재확인(토글 성공 검증용 — 북마크한 공고당 추가 호출 1회 발생).
...
- **이미 거른 회사:** `company.name`을 정규화(`㈜`·`(주)`·`주식회사`·영문 병기 괄호 제거) 후 `data/company-screening/*.md` 의 ❌거름 회사 목록과 대조해, 일치하면 제외.
