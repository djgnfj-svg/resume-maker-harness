...
| 2026-06-23 | discover-jobs **2차 — 사람인·잡코리아 어댑터 추가**, 4사 통합 검증 완료. 사람인=HTML 검색(`.item_recruit`: value/job_tit/corp_name/job_condition/job_date/job_sector)+스크랩 fetch `person-recruit-scrap-ajax`(status=scrap)/`-unset-ajax`(status=cancel, 응답 scrap_cnt). 잡코리아=SPA 검색(공고번호는 `GI_Read/{no}` 링크)+스크랩 버튼클릭(또는 `GI_Scrap` Scrap_State 1/0), 추적 POST의 개인정보는 미저장. 검증: 원티드 82·점핏 41 전수 북마크 + 사람인 6 스크랩, 잡코리아 메커니즘 확인 | skills/discover-jobs/references/{saramin,jobkorea}-discovery.md, SKILL.md 라우팅표 | 1차 검증 후 2차 확장 요청 |
| 2026-06-24 | discover-jobs **매칭 품질 정비**: `job-matcher` 서브에이전트 신설(공고 본문↔마스터 이력서 1:1 대조 → 높음/중간/낮음+근거). SKILL.md 7단계를 "키워드/정규식 등급 금지, 반드시 job-matcher 본문 실매칭(배치 병렬)"로 강제, 5단계는 객관적 1차 거름으로 한정, 8단계는 실매칭 결과만 북마크. 사유: 전수 처리에서 제목 키워드로 등급을 갈라 무관 공고까지 북마크된 사고 → 매칭이 핵심인데 누락됨 | agents/job-matcher.md, skills/discover-jobs/SKILL.md, resume-orchestrator(흐름 E) | "매칭 품질을 공고별 서브에이전트 비교로 제대로" 피드백 |
